...
HOSTNAME=dashboard-mfe.mfe-pot.local
...
helm --kube-context "kind-$CLUSTER_NAME" upgrade --install dashboard-mfe charts/dashboard-mfe \
...
kubectl --context "kind-$CLUSTER_NAME" rollout restart deployment/dashboard-mfe deployment/dashboard-bff
kubectl --context "kind-$CLUSTER_NAME" rollout status deployment/dashboard-mfe --timeout=60s
kubectl --context "kind-$CLUSTER_NAME" rollout status deployment/dashboard-bff --timeout=60s
